...
const handleRejectPromisse = (sqlErr, authErr, reject) => {
  if (sqlErr) {
    // SQL error ?
    throw sqlErr
  } else {
    // mark returned error object
    authErr.appAuthRemoved = true
  }
  reject(authErr)
}
const apiRequest = client => {
  const { dbFilename, db, table, axios } = client
              ...
              // https://developers.e-com.plus/docs/reference/store/#error-handling
                ...
                // remove current authentication
                  if (dbFilename) {
                    ...
                    const params = [myId]
                    // run query
                    db.run(sql, params, err => handleRejectPromisse(err, error, reject))
                  } else {
                    db.collection(table).doc(myId).delete().catch(err => handleRejectPromisse(err, error, reject))
                  }
                  return
                ...
                // wait few seconds and try again
                  ...
                  retry++
                  return
                } else if (status === 401 && authenticated && auth.row && config) {
